        ...
        if time_relationships < 0:
            ...
            if wait_answer() < 0:
                ...
                s_m(f'Я сейчас на исходе, поэтому бегом на '
                    f'место преступления. Это {data["point_1"]}\nКонец связи')
                time_talk_answer = True
            else:
                relationships_bot_police += 1
                s_m('Кратко ввожу в курс дела. Рядом произошёл суицид, сходи да проверь там всё')
                post_info('bot_police', 3, f'Чтобы через {data["time_1"]} мин был там, ок?!')
                relationships_bot_police += wait_answer()
        else:
            ...
            sleep(2)
            s_m(f'Тебе срочно надо выезжать. Место преступления {data["point_1"]}')
            sleep(1)
            s_m(f'Тебе даётся {data["time_1"]} мин, чтобы добраться до точки и просмотреть место происшествия')
            sleep(1)
    ...
    # Вторая развязка (4)
        ...
        if time_relationships < 0:
            ...
            s_m('Я сейчас на пределе, и если ты хочешь обсудить это, то лучше перенести это\n'
                ...
                f'Вот координаты, сам разберёшься "{data["coor_1"]}"')
        else:
            s_m(f'Значит неподалеку что-то случилось, выдвигайся туда\nУ тебя есть {data["time_1"]} мин. Вот координаты'
                f' "{data["coor_1"]}"')
