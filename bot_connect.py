...
def game(const):
    ...
    s_m('Произолшо очередное убийство! Это уже 5, пора заканчивать дело!!!', config.TOKEN_POLICE)
    sleep(2)
    s_m('Убит мужчина 27 лет в собственной квартире, в районе Весны Предположительная причина смерти:'
        ' передозировка инсулином.')
    sleep(1)
    s_m('Время смерти: 07.07.2020, 19:00')
    sleep(2)
    s_m('Выдвигайся к 149 школе на спортивную площадку')
    s_m('На этот раз у нас 10 мин')
    ...
    game_time = timer(10)  # на 10 минут, 5 левел
    ...
    for i in suspect:
        ...
        if i == 'Подруга, Инна':
            ...
            s_m('Парень')
            s_m(
                'Парень убитой, Ханин Олег Богданович, 24 года, 13.01.1996. Уроженец города '
